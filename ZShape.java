...
public class ZShape extends Shape {
	...
	public ZShape(int r, int c, Grid g) {
		/**
		 * Create an Z-Shape piece. See class description for actual location of
		 * r and c
		 * 
		 * @param r
	...
	private boolean canSquareMove(int rotation) {
		...
		if (rotation == 0 && square[1].canMove(Game.UP) && square[0].canMove(Game.UP) && square[2].canMove(Game.LEFT)) {
			return true;
		} else if (rotation == 1 && square[0].canMove(Game.DOWN) && square[1].canMove(Game.RIGHT)
				&& square[3].canMove(Game.RIGHT)) {

			return true;
		} else if (rotation == 2 && square[3].canMove(Game.UP) && square[2].canMove(Game.UP)
				&& square[0].canMove(Game.LEFT)) {

			return true;
		} else if (rotation == 3 && square[3].canMove(Game.RIGHT) && square[2].canMove(Game.DOWN)
				&& square[1].canMove(Game.RIGHT)) {
	...
	@Override
	/*
	 * @see Shape#rotate() override rotate method to support ZShape rotation
	 */
	public void rotate() {
		/*
		 * @param rotation count each case will check if canSquareMove is true
		 * if false ZShape will not rotate
		 */
